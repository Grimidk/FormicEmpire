...
echo "[Build] Checking Windows JRE (jre/)..."
chmod +x scripts/setup_jre.sh
...
pack_windows_zip() {
    ...
    if [ ! -d "jre" ] || [ ! -f "jre/bin/java.exe" ]; then
        echo "Error: jre/ not found. FormicEmpire.exe requires the bundled Windows JRE beside it."
        return 1
    fi
    rm -rf "$WIN_STAGING"
    mkdir -p "$WIN_STAGING"
...
if [ "$HAS_JAR_ZIP" -ne 1 ] || [ "$HAS_WIN_ZIP" -ne 1 ]; then
    echo ""
    echo "Error: JAR zip and Windows zip are required release artifacts."
    exit 1
fi
