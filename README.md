A game about ants developed solely on Java, autism, energy drinks, and cigarettes. 
I refuse to use an engine or learn proper UI/UX. 
Gemini is being used to help me debug, and all art is made using Aseprite with my own trackpad. 

Please enjoy it, and send feedback to: thegrimidk@gmail.com .
Check the roadmap.txt file to see planned features and known bugs. 

The game will eventually be published, but I don't care about profits.
I aim to publish the game around October 2026.

Use 'production' branch for a stable version and 'development' branch for unstable beta features.

Requirements:
    Java 17
    and Electricity (Optional)

Running the game (bash):
    ./run.sh
